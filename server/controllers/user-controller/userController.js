...
const handleUserSignup = asyncHandler(async (req, res, next) =>{
    ...
    if(!fname,!email,!password){
        const error = new AppError("Please enter the all the values", HttpStatusCodes.BAD_REQUEST)
    next(error) 
    }
...
// checking if the user allready exsit in database or not
 ...
 if(userExist){
    throw new AppError("User with this email address already exists", HttpStatusCodes.CONFLICT)
 }
